...
// Signup starts with a real sign-in. Carries trial / invite intent through to the
// API so the callback can flag the account or accept the invitation.
function RegisterAuthContent() {
  const params = useSearchParams();
  const isTrial = params.get('intent') === 'trial';
  const invite = params.get('invite');

  const oauthParams = new URLSearchParams();
  if (isTrial) oauthParams.set('intent', 'trial');
  if (invite) oauthParams.set('invite', invite);
  const qs = oauthParams.toString() ? `?${oauthParams.toString()}` : '';
    ...
    <div className="min-h-screen bg-[#FAFAF7] font-inter text-navy flex flex-col items-center justify-center p-6 text-center">
      <div className="bg-white p-10 md:p-12 rounded-[40px] shadow-xl border border-zinc-100 max-w-md w-full space-y-8">
        <div className="w-20 h-20 bg-gold/10 rounded-full flex items-center justify-center mx-auto text-gold">
          {isTrial ? <Sparkles className="w-10 h-10" /> : <ShieldCheck className="w-10 h-10" />}
        </div>

        <div className="space-y-3">
          <h1 className="font-playfair text-3xl font-black">{isTrial ? 'Start your free trial' : 'Create your account'}</h1>
          <p className="text-zinc-500">
            {isTrial
              ? 'Sign in to explore your dashboard. You can finish setting up your guide anytime.'
              : 'Sign in to begin setting up your family guide.'}
          </p>
        </div>
